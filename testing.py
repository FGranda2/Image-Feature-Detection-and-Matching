...
# sample_list = ['000011'] # Picture by picture analysis
...
for sample_name in sample_list:
    ...
    # Draw left images with 1000 Keypoints. (Uncomment to draw)
    #img_left2 = cv.drawKeypoints(img_left, kp_left, None, color=(0, 255, 0), flags=0)  # (Uncomment to draw)
    #plt.imshow(img_left2), plt.show()  # (Uncomment to draw)
    ...
    # Draw images with filtered matches(Uncomment to draw)
    #draw_params = dict(matchesMask=matchesMask,    # (Uncomment to draw)
    #                    flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)  # (Uncomment to draw)
    #img3 = cv.drawMatchesKnn(img_left, kp_left, img_right, kp_right, matches, None, **draw_params) # (Uncomment to draw)
    #plt.imshow(img3), plt.show()   # (Uncomment to draw)
    ...
    # Draw images with filtered matches (Uncomment to Draw)
    # draw_params = dict(matchesMask=mask,  # (Uncomment to draw)
    #                    flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)  # (Uncomment to draw)
    # img4 = cv.drawMatchesKnn(img_left, kp_left, img_right, kp_right, matches, None, **draw_params)# (Uncomment to draw)
    # plt.imshow(img4), plt.show()  # (Uncomment to draw)
    ...
    # Output
    for u, v, dispar, depth in zip(np.int32(left_final[:, 0]), np.int32(left_final[:,1]), np.int32(disp2), depth_values2):
        line = "{} {:.2f} {:.2f} {:.2f} {:.2f}".format(sample_name, u, v, dispar, depth)
